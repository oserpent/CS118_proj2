        ...
        while(1) {
            ...
            if (n > 0) {
                ...
                if (idx >= 0) {
                    ...
                    if (idx == s) {     // COMMENT: If it's not the first pkt that is received, then no need to do much more with regards to window. Else, do the following.
                    		bool flag = true;
                    		int temp = getFirstNonRcvdIdx(s, e, &rcvd);
                        int i = s;
                        if (temp != -1) {     // COMMENT: First nonreceived pkt is within the window.
                        		while (i != temp || (flag && i == temp)) {     // DESCRIPTION: saves consecutively received pkts
                        				flag = false;
                        				// (INSERT CODE FOR SAVING pkts[i] HERE)
                            		i = (i + 1) % WND_SIZE;
                        		}
                    				s = temp;
                        ...
                        else {     // COMMENT: When s is the last pkt received and all other pkts have been received already.
                            while (i != e || (flag && i == e)) {     // DESCRIPTION: saves consecutively received pkts
                        				flag = false;
                        				// (INSERT CODE FOR SAVING pkts[i] HERE)
                            		i = (i + 1) % WND_SIZE;
                        		}
